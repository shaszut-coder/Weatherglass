...
- **Category is gone entirely (v5.0.0), not deprecated, not hidden.** The finding that started this: with quick capture as the dominant real-world workflow, "Entry Type" and "Category" weren't just under-filled, they were *actively wrong* — both defaulted to fixed values (`level:"Routine"`, `category:"Temperature"`) regardless of real conditions, and were essentially never revisited afterward. Concretely, this meant real Severe/Extreme-alert entries (including an actual tornado warning) were silently saved as "Routine," undercounting the Severe Events feature this whole time. **Level is now auto-derived from each entry's own real alert data** on quick captures (no alert → Routine, active non-severe alert → Notable, active Severe/Extreme alert → Severe) — manual entry keeps full deliberate control over all three, since the derivation only replaces guessing, not judgment (a beautiful alert-free sky can still be manually marked Notable). **Category had no honest equivalent** — nothing in a quick capture signals Wind vs. Sky vs. Storm — so it was removed outright rather than left as a guessed default: from the form, the Journal filter, CSV export, and every place that read it (Gallery's sky view and Sky Statistics now key off `cloudGenus` instead, which turned out to be a more accurate signal than category ever was). A retroactive cleanup tool (Settings → Data Cleanup) re-derives Level and clears the legacy Category field on existing quick-capture entries specifically, leaving anything set manually untouched — this is the precedent for how to handle a schema change to real existing data: explicit, user-triggered, scoped narrowly, never a silent migration.
...
- **Cloud classification: the guided, non-AI picker shipped first (v4.9.0), and real AI classification followed (v4.10.0)** once the picker turned out to still be genuinely hard for someone still building the skill to recognize cloud types from memory — a legitimate finding from actual use, not a change of mind for its own sake. The original objection (AI would need a client-exposed API key, breaking the app's free/keyless pattern everywhere else) was real and still holds — it's just no longer the only option. The resolution: AI is opt-in and routes through a self-hosted Cloudflare Worker the user deploys and controls, holding their own Anthropic API key as a private server-side secret. This is the first real backend component the app has — client-only stopped being a hard rule and became "true everywhere except this one deliberately-chosen exception." The guided picker and manual dropdown remain fully available regardless of whether AI is connected.
...
- **The cloud picker shipped (v4.9.0), but in a different shape than originally designed** — worth recording precisely, since it landed differently for a good reason. The original plan put it inside the Manual Entry/Edit overlay as the default classification path for new entries. What actually got built first: a standalone **review queue** (Settings → Classify Cloud Photos) that scans for already-existing photos missing a cloud type and lets you tag them in a fast loop, using the same texture-first dichotomous key (2 taps for most of the 10 WMO genera). The prompt that led to building it was specifically about clearing a backlog of unclassified photos, not the moment-of-capture flow — so that's what got built. Wiring the same picker into the Manual Entry form itself, as originally planned, is still open. The "life list" extension to `renderMyCloudAtlas()` (showing genera not yet spotted) also remains unbuilt.
